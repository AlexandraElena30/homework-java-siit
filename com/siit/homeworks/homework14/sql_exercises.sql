...
# 2. Să se selecteze numele, funcția și salariu pentru angajații care au aceași funcție.
 ...
 select group_concat(ENAME separator ', '),JOB,group_concat(SAL separator ', ') as salaries,avg(SAL)
    from EMP e
...
#select concat (EMPNO,' ', ENAME) ,JOB,HIREDATE
#from emp
#ORDER BY EMPNO desc ;
...
#select concat (EMPNO,' ', ENAME),JOB,SAL, null as semnatura
#from emp
#order by DEPTNO;
...
#from emp
#where EMPNO = 7499 or EMPNO = 7902 or EMPNO = 7876;
...
#select ENAME, (SAL + ifnull(COMM, 0)) as venit_fara_comision
    #from EMP
   # where SAL + ifnull(COMM, 0) > 2000;
